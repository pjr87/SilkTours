/*
 * MessageConstants
 * These are the variables that contain the text which is displayed on certain errors
 *
 * Follow this format:
 * export const YOUR_CONSTANT = 'Your message text';
 */
export const FIELD_MISSING = 'Please fill out the entire form.';
export const WRONG_PASSWORD = 'Wrong password.';
export const USER_NOT_FOUND = 'This username does not exist.';
export const DATABASE_ERROR = 'Error encountered when connecting to database.';
export const PHONE_NUMBER_INVALID = 'Phone number must contain 10 or 12 digits.';
export const SIGNUP_FAILED = 'Sign Up Failed.';
export const USER_UPDATE_FAILED = 'Failied to properly update user.';
export const USERNAME_TAKEN = 'Sorry, but this username is already taken.';
export const OTHER_PROVIDER = 'Sorry, user account is logged in with another provider.';
export const GENERAL_ERROR = 'Something went wrong, please try again.';
export const TOUR_NOT_FOUND = 'No tour found.';
